...
type pp_token =
  ...
  | Pp_break of {              (* complete break *)
      no_break : string;          (* print this if no break *)
      yes_break : string * string (* print fst, break, then print snd *)
    }
...
let break_new_line state (before_text, after_text) width =
...
let break_line state width = break_new_line state ("", "") width

(* To format a break that fits on the current line. *)
let break_same_line state no_break_text =
...
let format_pp_token state size = function
  ...
  | Pp_tbreak (n, off) ->
    ...
    | Some (Pp_tbox tabs) ->
      ...
      if offset >= 0
      then break_same_line state (spaces (offset + n))
      else break_new_line state ("", spaces (tab + off)) state.pp_margin
    end
...
let pp_print_break state width offset =
  if state.pp_curr_depth < state.pp_max_boxes then
    let size = Size.of_int (- state.pp_right_total) in
    let token = Pp_break {
      no_break = spaces width;
      yes_break = ("", spaces offset);
    } in
