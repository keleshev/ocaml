...
let pop_ignore stack = try ignore (Stack.pop stack) with Stack.Empty -> ()
...
module DefaultStack (DEFAULT : sig type t val default : t end) : sig
  ...
  val create : unit -> DEFAULT.t t
  val clear : DEFAULT.t t -> unit
  val top : DEFAULT.t t -> DEFAULT.t
  val pop : DEFAULT.t t -> DEFAULT.t
  val push : DEFAULT.t -> DEFAULT.t t -> unit

end = struct
  include Stack

  let top stack = try Stack.top stack with Stack.Empty -> DEFAULT.default
  let pop stack = try Stack.pop stack with Stack.Empty -> DEFAULT.default
end
...
module ScanStack = DefaultStack (struct
  type t = pp_scan_elem

  let default =
    let queue_elem = { size = Size.unknown; token = Pp_text ""; length = 0 } in
    { left_total = -1; queue_elem }
end)
...
type formatter = {
  (* The pretty-printer scanning stack. *)
  pp_scan_stack : pp_scan_elem ScanStack.t;
  (* The pretty-printer formatting stack. *)
...
let pp_force_break_line state =
  match Stack.top state.pp_format_stack with
  | exception Stack.Empty -> pp_output_newline state
  | { box_type; width } ->
...
let format_pp_token state size = function
  ...
  | Pp_end ->
    pop_ignore state.pp_format_stack (* Empty -> no more boxes to close. *)

  | Pp_tbegin (Pp_tbox _ as tbox) ->
    Stack.push tbox state.pp_tbox_stack

  | Pp_tend ->
    pop_ignore state.pp_tbox_stack (* Empty -> no more boxes to close. *)

  | Pp_stab ->
    begin match Stack.top state.pp_tbox_stack with
    | Pp_tbox tabs ->
      let rec add_tab n = function
        | [] -> [n]
        | x :: l as ls -> if n < x then n :: ls else x :: add_tab n l in
      tabs := add_tab (state.pp_margin - state.pp_space_left) !tabs
    | exception Stack.Empty -> () (* No open tabulation box. *)
    end
  ...
  | Pp_newline ->
    begin match Stack.top state.pp_format_stack with
    | { width; _ } -> break_line state width
    | exception Stack.Empty -> pp_output_newline state (* No open box. *)
    end
  ...
  | Pp_break (n, off) ->
    ...
    | exception Stack.Empty -> () (* No open box. *)
    end
   ...
   | Pp_close_tag ->
     begin match Stack.pop state.pp_mark_stack with
     | tag_name ->
       let marker = state.pp_mark_close_tag tag_name in
       pp_output_string state marker;
     | exception Stack.Empty -> () (* No more tag to close. *)
     end
...
let rec advance_loop state =
  let { size; token; length } = Queue.peek state.pp_queue in
  if not (Size.is_unknown size &&
...
(* Setting the size of boxes on scan stack:
   ...
   Note:
   Pattern matching on token in scan stack is exhaustive,
   since scan_push is only used on breaks and opening of boxes. *)
let set_size state ty =
  let { left_total; queue_elem } = ScanStack.top state.pp_scan_stack in
  let size = Size.to_int queue_elem.size in
  (* test if scan stack contains any data that is not obsolete. *)
  if left_total < state.pp_left_total then
    ScanStack.clear state.pp_scan_stack
  else
    match queue_elem.token with
    | Pp_break (_, _) | Pp_tbreak (_, _) ->
      if ty then begin
        queue_elem.size <- Size.of_int (state.pp_right_total + size);
        ScanStack.pop state.pp_scan_stack |> ignore
      end
    | Pp_begin (_, _) ->
...
let scan_push state b tok =
  ...
  let elem = { left_total = state.pp_right_total; queue_elem = tok } in
  ScanStack.push elem state.pp_scan_stack
...
let pp_close_tag state () =
  ...
  if state.pp_print_tags then
    match Stack.pop state.pp_tag_stack with
    | tag_name ->
...
let pp_set_formatter_tag_functions state tag_functions =
  state.pp_mark_open_tag <- tag_functions.mark_open_tag;
  state.pp_mark_close_tag <- tag_functions.mark_close_tag;
  state.pp_print_open_tag <- tag_functions.print_open_tag;
  state.pp_print_close_tag <- tag_functions.print_close_tag
...
let pp_set_formatter_out_functions state out_functions =
  state.pp_out_string <- out_functions.out_string;
  state.pp_out_flush <- out_functions.out_flush;
  state.pp_out_newline <- out_functions.out_newline;
  state.pp_out_spaces <- out_functions.out_spaces;
  state.pp_out_indent <- out_functions.out_indent
...
let pp_make_formatter f g h i j =
  ...
  let sys_scan_stack = ScanStack.create () in
  ScanStack.push { left_total = 1; queue_elem = sys_tok} sys_scan_stack;
  let pp_margin = 78
