...
2 |   type t = string [@@immediate]
      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: The attributes attached to this type indicate that it is always an
       immediate type. However, it is really not an immediate type.
       Types marked with the immediate attribute must be non-pointer types
       like int or bool.
|}];;
